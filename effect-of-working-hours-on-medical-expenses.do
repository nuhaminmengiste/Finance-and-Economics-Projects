...
/// The t- distriubtion will have a mean of 0 and a standard error of 1. This is because the sample size is large enough for the distribution to be like the normal one.  If the null hypothesis is true, the t-value we get will be less than the critical value calculated below and the probability of getting that t-value must be more than the specified alpha below. Note: the regression gives us the t value if the true value of B2 is 0. I do not need to specify this because it calculates the t value on this assumption by default.
